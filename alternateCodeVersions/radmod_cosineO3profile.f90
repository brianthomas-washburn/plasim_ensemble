...
!            za(jh1:jh2)=a0o3+a1o3*ABS(sid(jlat))                        &
            !BT 21June2023 - changing sin to cos in latitude here to modify O3 profile,
            ! for project looking at climate effects of modified O3 profiles,
            ! specifically simulating (simplified) SNCR profiles.
            ! Here, cos will give max around equator as seen in 20 pc SNCR case
            za(jh1:jh2)=a0o3+a1o3*ABS(cola(jlat))                        &
!              +aco3*sid(jlat)*cos(TWOPI*(zcday-toffo3)                 &
                 +aco3*cola(jlat)*cos(TWOPI*(zcday-toffo3)                 &
...
!
      parameter(nunit=37)
      character(len=19) :: yf             ! filename 
      integer :: ih(8)                    ! SERVICE header
      integer :: kcheck(NLEV,0:13)        ! check data
      real    :: zin(NLON*NLAT,NLEV,0:13) ! input
      logical :: lex                      ! exists
!
      if(mypid == NROOT) then
       write(yf,'("N",I3.3,"_level_",I4.4,".sra")') NLAT,237
       inquire(file=yf,exist=lex)
       if(lex) then
        open(nunit,file=yf,form='formatted')
        kcheck(:,:)=0 
        do   
         read(nunit,*,iostat=io) ih(:)
         if(io /= 0) exit
         kmon=ih(3)
         if(kmon >= 100) kmon=mod(kmon/100,100)
         klev=ih(2)
         if(kmon<0.or.kmon>13.or.klev<1.or.klev>NLEV) then
          write(nud,*) 'Inconsisten O3-file',trim(yf)
          write(nud,*) 'Unexpected month or level'
          call mpabort('ERROR reading O3 file')
         endif
         read(nunit,*,iostat=io) zin(:,klev,kmon)
         if(io /= 0) then
        ...
        close(nunit)
        kmons=SUM(kcheck(1,:))
        klevs=SUM(kcheck(:,:))/kmons
        if(klevs == NLEV) then
         if(kmons == 14) then
          write(nud,*) 'O3 for months 0-13 read from ',yf
         elseif(kmons == 1) then
          do jm=0,13
           if(SUM(kcheck(:,jm)) == NLEV) then
            do jm2=0,13
             zin(:,:,jm2)=zin(:,:,jm)
            enddo
            write(nud,*) 'O3 for month ',jm,' read from ',yf
            write(nud,*) 'O3 expanded to months 14 months'
           endif
          enddo
         elseif(kmons == 12) then
          zin(:,:,0)=zin(:,:,12)
          zin(:,:,13)=zin(:,:,1)
          write(nud,*) 'O3 for 12 months read from ',yf
          write(nud,*) 'Expanded O3 to 14 (0-13) months'
         else
          write(nud,*) 'ERROR in O3-file',trim(yf)
          write(nud,*) 'Unexpected number of months(levels)'
          call mpabort('ERROR reading O3 file')    
         endif
        else
          write(nud,*) 'Inconsisten O3-file',trim(yf)
          write(nud,*) 'Inconsisten number of months/levels'
          call mpabort('ERROR reading O3 file')
        endif  
       else
        write(nud,*) 'WARNING: No O3-Input found'
        write(nud,*) 'O3 set to Zero'
       endif
      endif   
      call mpscgp(zin,dqo3cl,14*NLEV)
!
      return
      end subroutine get_o3
